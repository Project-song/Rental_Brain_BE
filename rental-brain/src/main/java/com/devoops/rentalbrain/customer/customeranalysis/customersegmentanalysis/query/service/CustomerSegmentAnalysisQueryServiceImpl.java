...
//    }
    ...
    public CustomerSegmentDetailCardDTO getSegmentDetailCard(long segmentId) {
        ...
        CustomerSegmentDetailCardDTO card =
                customerSegmentAnalysisQueryMapper.getSegmentDetailCard(segmentId); // Mapper 호출 1회로 줄임
        ...
        // null + 빈칸 방어
        card.setTopItemName((card.getTopItemName()
                == null || card.getTopItemName().isBlank()) ? "-" : card.getTopItemName());
        card.setTopSupport((card.getTopSupport()
                == null || card.getTopSupport().isBlank()) ? "-" : card.getTopSupport());
        card.setTopFeedback((card.getTopFeedback()
                == null || card.getTopFeedback().isBlank()) ? "-" : card.getTopFeedback());
